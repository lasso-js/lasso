...
function buildManifest(input, context) {
    var templateResource = input.templatePath;

    if (input.packagePath) {
        throw new Error('input.packagePath not yet supported');
        // var packageFile = templateResource ?
        //     templateResource.resolve(input.packagePath) :
        //     resources.findResource(input.packagePath);
                
        // if (!packageResource.exists()) {
        //     throw new Error('Unable to configure page optimizer. The package resource at path "' + packageResource.getPath() + '" does not exist.');
        // }
        // return packaging.readPackage(packageResource);
    }
    else {
...
module.exports = {
    process: function(input, context) {
        ...
        function doOptimizePage() {
            ...
            optimizedPage = pageOptimizer.optimizePageCached(
                optimizerContext, 
                cacheKey,
                function() {
        ...
        if (waitFor && waitFor.length) {
            ...
            optimizedPage = promises.all(waitFor)
                .then(doOptimizePage);
        }
        else {
