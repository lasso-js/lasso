...
function getOptimizerManifestFromOptions(options, dependencyRegistry) {
    ...
    if (options.manifest) {
        ...
        if (optimizerManifest) {
            optimizerManifest = new OptimizerManifest(optimizerManifest, dependencyRegistry, optimizerManifest.__dirname, optimizerManifest.__filename);
        }
    }
...
PageOptimizer.prototype = {
    ...
    buildPageBundles: function(options, context) {
        ...
        return buildBundleMappings()
            .then(function(bundleMappings) {
                console.log('Bundle mappings built in ' + (Date.now() - startTime) + 'ms');
    ...
    optimizePage: function(options) {
        ...
        if (context) {
            if (!(context instanceof OptimizerContext)) {
                context = raptorUtil.extend(new OptimizerContext(), context);
            }
        }
        else {
    ...
    resolveResourceUrlCached: function(path, context) {
        var cache = this.getCache(context);
        var _this = this;
        return cache.getResourceUrl(path, function() {
