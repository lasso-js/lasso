...
var OptimizerWriterMixins = require('./OptimizerWriterMixins');
var OptimizerFileWriter = require('./OptimizerFileWriter');
...
PageOptimizer.prototype = {
    ...
    getWriter: function(options) {
        var writer = (options && options.writer) || this.writer || this.createWriter();
        if (!writer.__OptimizerWriterMixins) {
            OptimizerWriterMixins.addMixins(writer);
            writer.configure(this);
        }
    ...
    applyFilters: function(code, contentType, context) {
        return filters.applyFilters(code, contentType, context);
    },
    ...
    readDependency: function(dependency, context) {
        ...
        var filterContext = context ? extend({}, context) : {};
        ...
        function filterCode() {
            ...
            dependencies.forEach(function(dependency, i) {
                ...
                promiseChain = promiseChain
                    .then(function() {
                        return dependency.getCode(context);
                    })
                    .then(function(code) {
                        return _this.applyFilters(code, contentType, filterContext)
                            .then(function(code) {
                                // Put the filtered code back into the array
            ...
            return promiseChain
                .then(function() {
                    return filteredCodeArray.join("\n");
                });
        }


        // Step 2
        function finish(bundleCode) {
     ...
     */
    resolveResourceUrl: function(path, from, context) {
        ...
        if (inPlaceFromDir && config.isInPlaceDeploymentEnabled()) {
            ...
            var inPlacePath = writer.urlBuilder.getInPlaceResourceUrl(
                path, 
                inPlaceFromDir);

            if (inPlacePath) {
                inPlacePath += queryString + hashString;
                logger.debug("Resolved URL: " + inputPath + ' --> ' + inPlacePath);
                return raptorPromises.makePromise(inPlacePath);
            }
        }
            ...
            return writer.writeResource(path, context)
                .then(function(outputFileInfo) {
                    ...
                    if (relativeFromDir && outputFileInfo.file) {
                        ...
                        // a relative path from the optimizer output directory (e.g. where this CSS file will reside)
                        // to the output image/resource
                        url = nodePath.relative(
                            relativeFromDir, 
                            outputFileInfo.file.getAbsolutePath()) + queryString + hashString;
                    }
                    else if (outputFileInfo.url) {
                        url = outputFileInfo.url;

                        if (url.startsWith("data:") === false) {
                            url = url + queryString + hashString;    
                        }
                    }
                    else {
        ...
        return raptorPromises.makePromise(path);
    }
};
