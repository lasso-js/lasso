...
function getOptimizerManifestFromOptions(options, dependencyRegistry, callback) {
    ...
    if (lassoManifestOptions) {
        callback(null, new OptimizerManifest(lassoManifestOptions));
    } else {
        callback(null, null);
    }
}

function doOptimizePage(pageOptimizer, options, lassoContext, callback) {
    ...
    getOptimizerManifestFromOptions(options, pageOptimizer.dependencies, function(err, lassoManifest) {
        if (!lassoManifest) {
        ...
        var asyncTasks = [
            function buildPageBundles(callback) {
                pageOptimizer.buildPageBundles(options, lassoContext, function(err, _pageBundles) {
                    if (err) {
                        return callback(err);
                    }
...
function doOptimizeResource(pageOptimizer, path, options, lassoContext, callback) {
...
PageOptimizer.prototype = {
    ...
    getAppBundleMappingsCached: function(bundleSetConfig, lassoContext, callback) {
        var lassoCache = this.getOptimizerCache(lassoContext);
    ...
    buildOptimizerCacheKey: function(lassoContext) {
     ...
     * @return {OptimizerCache} the lasso cache associated with this page lasso
     */
    getOptimizerCache: function(lassoContext) {
        var cache = lassoContext.cache;
        if (!cache) {
            var config = this.getConfig();

            var key = this.buildOptimizerCacheKey(lassoContext);
            cache = this.lassoCacheLookup[key];
            if (!cache) {
                cache = this.lassoCacheLookup[key] = new OptimizerCache(key, {
     ...
     * - lasso: A reference to the PageOptimizer
    ...
    createOptimizerContext: function(options) {
        ...
        lassoContext.cache = this.getOptimizerCache(lassoContext);
    ...
    optimizePage: function(options, callback) {
        ...
        var lassoContext = options.lassoContext || this.createOptimizerContext(options);
        ...
        if (options.cache !== false) {
            ...
            lassoCache.getOptimizedPage(
                cacheKey,
                {
                    builder: function(callback) {
                        ...
                        doOptimizePage(_this, options, lassoContext, callback);
                    }
                },
                done);
            ...
            doOptimizePage(_this, options, lassoContext, done);
        }
    ...
    optimizeResource: function(path, options, callback) {
        ...
        if (!lassoContext) {
            lassoContext = options.lassoContext || this.createOptimizerContext(options);
        }

        var _this = this;

        if (options.cache !== false) {
            var cache = this.getOptimizerCache(lassoContext);
            var cacheKey = path;
            ...
            cache.getOptimizedResource(
                cacheKey,
                function(callback) {
                    doOptimizeResource(_this, path, options, lassoContext, callback);
                },
                callback);
        } else {
            doOptimizeResource(_this, path, options, lassoContext, callback);
