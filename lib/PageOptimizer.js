...
PageOptimizer.prototype = {
        ...
        var promise = this.buildPageBundles(options, context)
            .then(function(_pageBundles) {
                if (logger.isInfoEnabled()) {
                    logger.info('Page bundles built in ' + (Date.now() - prevStartTime) + 'ms');
                }
            ...
            .then(function() {
                if (logger.isInfoEnabled()) {
                    logger.info('Async page bundles written in ' + (Date.now() - prevStartTime) + 'ms');
                }
                prevStartTime = Date.now();
            ...
            .then(function() {
                if (logger.isInfoEnabled()) {
                    logger.info('Page bundles written in ' + (Date.now() - prevStartTime) + 'ms');
                }
    ...
    optimizeResourceCached: function(path, context, callback) {
        ...
        if (context.basePath) {
            cacheKey += '|' + context.basePath;
        }

        return cache.getOptimizedResource(
            cacheKey,
            {
                builder: function() {
                    return _this.optimizeResource(path, context);
                }
            },
            callback);
    },
