...
function doOptimizePage(pageOptimizer, options, callback) {
    ...
    var optimizerManifest = getOptimizerManifestFromOptions(options, pageOptimizer.dependencies);
    if (!optimizerManifest) {
    ...
    var promise = pageOptimizer.buildPageBundles(options, optimizerContext)
        .then(function(_pageBundles) {
            ...
            return writer.writeBundles(pageBundles.forEachAsyncBundleIter(), onAsyncBundleWritten, optimizerContext);
        })
        .then(function() {
            if (perfLogger.isInfoEnabled()) {
            ...
            return writer.writeBundles(pageBundles.forEachBundleIter(), onBundleWritten, optimizerContext);
        })
        .then(function() {
            if (perfLogger.isInfoEnabled()) {
...
PageOptimizer.prototype = {
    ...
    buildPageBundles: function(options, optimizerContext) {
        ...
        return deferred.promise.then(function(bundleMappings) {
                ...
                // We clear out the attributes because we put temporary data into the attributes
                // to avoid repeating the same work and adding the same dependencies multiple times.
                // After we build global bundlers, we need to reset the attributes so that we will
                // make sure all required dependencies are added to the page bundles
                optimizerContext.clearAttributes();
    ...
    optimizePage: function(options, callback) {
        ...
        if (options.cache !== false) {
            ...
            return doOptimizePage(_this, options, callback);
        }
    },
