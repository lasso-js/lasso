...
Transformer.prototype = {
    ...
    transform: function (inStream, optimizerContext) {
        ...
        function applyTransform(input, transform, callback) {
            return transform.transform(input, optimizerContext, callback);
        }
...
exports.createTransformer = function(unfilteredTransforms, optimizerContext, callback) {
    ...
    function filter(transformConfig, callback) {
        if (transformConfig.filter) {
            transformConfig.filter(optimizerContext, function(err, keep) {
                if (err) {
                    return done(err);
                }
