...
function build(options, config, bundleMappings, lassoContext, callback) {
    ...
    function processSyncDependency(dependency, walkContext, debugTree) {
    ...
    var buildAsyncPackagesWorkQueue = require('raptor-async/work-queue').create({
        ...
        // this function is called when we need to run task to handle
        // async package
        ...
        return dependencyWalker.walk({
                ...
                on: {
                    ...
                    dependency: function(dependency, walkContext) {
                        ...
                        if (bundle) {
                            // We found an asynchronous bundle so we need to
                            // make sure that we create a loader metadata entry.
                            // Since we creating a new loader-metadata Dependency
                            // while walking, we manually process this new Dependency
                            // to make sure it gets put into a synchronous
                            // page bundle.

                            // de-duplicate the loader metadata entry
                            var loaderMetadataKey = asyncPackageName + '|' + bundle.getKey();
                            if (!seenLoaderMetadata[loaderMetadataKey]) {
                                seenLoaderMetadata[loaderMetadataKey] = true;

                                // Create a dependency on the fly that represents
                                // the loader metadata entry that will cause
                                // this bundle to become part of the given
                                // async package.
                                var loaderMetadataDep = lasso.dependencies.createDependency({
                                    type: 'loader-metadata',
                                    packageName: asyncPackageName,
                                    bundle: bundle
                                }, lassoContext.getProjectRoot() /* dirname */, null /* filename */);

                                // The _key is synchronously generated here
                                // to bypass some of the async flow that
                                // would normally be used when calculating
                                // key for dependency
                                loaderMetadataDep._key = 'loaderMetadata:' + loaderMetadataKey;

                                var nestedWalkContext = Object.create(walkContext);
                                nestedWalkContext.slot = 'body';
                                nestedWalkContext.dependency = loaderMetadataDep;

                                // Always process loader metadata entries as
                                // synchronous dependencies (so that they don't
                                // get put into async packages)
                                processSyncDependency(loaderMetadataDep,
                                    // Create a fake WalkContext
                                    nestedWalkContext,
                                    debugTree);
                            }
        ...
        dependencyWalker.walk({
                ...
                on: {
                    ...
                    dependency: function(dependency, walkContext) {
                        ...
                        processSyncDependency(dependency, walkContext, debugTree);
                    }
                }
            },
            function(err) {
