...
function build(options, config, bundleMappings, lassoContext, callback) {
    ...
    assert.ok(OptimizerManifest.isOptimizerManifest(lassoManifest), '"lassoManifest" is not a valid package');
    ...
    var buildAsyncPackagesWorkQueue = require('raptor-async/work-queue').create({
        ...
        var nestedOptimizerContext = Object.create(lassoContext);
        ...
        return dependencyWalker.walk({
            ...
            flags: flags,
            lassoContext: nestedOptimizerContext,
            on: {
