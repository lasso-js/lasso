...
BundleMappings.prototype = {
    ...
    addDependencyToBundle: function(dependency, targetBundleName, dependencySlot, bundleConfig) {
        ...
        var inline = dependency.inline;

        if (inline === true || inline === 'merge') {
            inline = INLINE_END;
        } else if (inline === 'in-order') {
            inline = INLINE_IN_ORDER;
        } else {
            inline = 0;
        }

        var bundleKey = Bundle.getKey(dependencySlot, dependency.getContentType(), inline, targetBundleName);
        ...
        if (!targetBundle) {
            ...
            targetBundle.setInline(inline !== 0);
            targetBundle.setMergeInline(inline === INLINE_END);
