...
BundleMappings.prototype = {
    ...
    addDependencyToBundle: function(dependency, targetBundleName, dependencySlot, bundleConfig, optimizerContext) {
    ...
    addDependencyToPageBundle: function(dependency, pageBundleName, dependencySlot, bundleConfig, optimizerContext) {
        ok(optimizerContext, 'optimizerContext expected');
        ...
        if (this.inPlaceDeploymentEnabled && dependency.isInPlaceDeploymentAllowed()) {
            ...
            if (dependency.getUnbundledTarget) {
                relPath = dependency.getUnbundledTarget(optimizerContext) + '-' + pageBundleName;
            }
            ...
            if (!relPath) {
                relPath = dependency.type + '-' + pageBundleName;
            }

            bundle = this.addDependencyToBundle(
                dependency,
                relPath || dependency.getKey(),
                dependencySlot,
                null,
                optimizerContext);
