...
FileWriter.prototype = {
    ...
    checkBundleUpToDate: function(bundle, context) {
        return; // Always write the bundle
    ...
    doWriteBundle: function(input, bundle) {
        ...
        var calculateChecksum = checksumsEnabled && !bundle.getChecksum();
        ...
        this._writeFile(input, outputFile, calculateChecksum, function(err, result) {
            if (calculateChecksum) {
                var tempFile = result.tempFile;
                var checksum = result.checksum;

                ok(tempFile, 'expected tempFile');
                ok(checksum, 'expected checksum');

                bundle.setChecksum(checksum);
                outputFile = _this.getOutputFileForBundle(bundle);
    ...
    doWriteResource: function(input, path, context) {
        ...
        var outputFile = this.getOutputFileForResource(path);


        var calculateChecksum = this.checksumsEnabled === true;
        var _this = this;
        ...
        this._writeFile(input, outputFile, calculateChecksum, function(err, result) {
            if (calculateChecksum) {
                ...
                outputFile = _this.getOutputFileForResource(path, checksum);
    ...
    _writeFile: function(inStream, outputFile, calculateChecksum, callback) {
        ...
        mkdirp(outputDir, function(err) {
            ...
            if (calculateChecksum) {
                ...
                inStream
                    ...
                    .pipe(_this.getFileOutputStream(tempFile))
                    .on('error', callback)
                    .on('close', function() {
                        callback(null, {
                            tempFile: tempFile,
                ...
                inStream
                    .pipe(_this.getFileOutputStream(outputFile))
                    .on('error', callback)
                    .on('close', function() {
                        callback(null, {});
                    });
