    ...
    getOutputFile: function(sourceFile, filename, checksum, targetExt, slotName) {
        ...
        if (sourceFile) {
            if (projectRoot && sourceFile.startsWith(projectRoot)) {
                ...
                // Let's rebase the module at a root package
                ...
                if (modulePkg) {
                    ...
                    outputPath = nodePath.join(this.outputDir, name + '-' + version + '/' + sourceFile.substring(modulePkg.__dirname.length));
                }
            }
        }
