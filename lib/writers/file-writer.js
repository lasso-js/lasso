...
/**
 * Calculate the last modified time for a file and
 * return -1 if there were any errors while trying
 * to stat the file
 *
 * @param  {String}   path     The file path
 * @param  {Function} callback The callback function
 */
function lastModified(path, callback) {
        ...
        checkResourceUpToDate: function(path, context, callback) {
            ...
            var work = {
                sourceLastModified: function(callback) {
                    lastModified(path, callback);
                },
                outputLastModified: function(callback) {
                    lastModified(outputFile, callback);
                }
            };
