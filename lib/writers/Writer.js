...
Writer.prototype = {
    ...
    writeBundle: function(bundle, onBundleWrittenCallback) {
        ...
        function onBundleFinished() {
            if (!bundle.isInline() && !bundle.getUrl()) {
        ...
        return resolved
            .then(function() {
                if (_this.checkBundleUpToDate) {
                    return _this.checkBundleUpToDate(bundle, context);
                }
            })
            .then(function() {
                if (bundle.getUrl()) {
                    // After checking a URL was added to the bundle so we don't
                    // need to actually read and write the bundle
