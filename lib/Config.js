...
var lassoImagePlugin = require('lasso-image');
...
function calculateConfigFingerprint(config) {
    ...
    config._plugins.forEach(function(pluginConfig) {
        ...
        update(pluginConfig.config);
    });

    if (config.fileWriterConfig) {
        ...
        update(config.fileWriterConfig.relativeUrlsEnabled);
    }

    update(config.bundleSetConfigsByName);
