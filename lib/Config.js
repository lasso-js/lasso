...
Config.prototype = {
    ...
    enableJSMinification: function() {
        this.addFilter(require("./filters/MinifyJSFilter"));
    },

    enableCSSMinification: function() {
        this.addFilter(require("./filters/MinifyCSSFilter"));
    },

    enableResolveCSSUrlFilter: function() {
        this.addFilter(require("./filters/ResolveCSSUrlsFilter"));
    },
    ...
    addFilter: function(filter) {
        ...
        for (var i=0; i<this.filters.length; i++) {
            if (this.filters[i].filter === filter.filter) {
                return;
            }
        }
    ...
    getUrlForSourceFile: function(sourceFilePath) {
        ...
        for (var i=0, len=this.serverSourceMappings.length; i<len; i++) {
            ...
            if (sourceFilePath.startsWith(mapping.baseDir)) {
        ...
        bundleSetConfig.forEachBundleConfig(function(bundleConfig) {
            promiseChain = promiseChain.then(function() {
                ...
                return bundleMappings.addDependenciesToBundle(
                    ...
                    bundleName,
                    bundleConfig.checksumsEnabled,
                    bundleConfig.wrappers);
            });
        });
