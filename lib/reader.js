...
function createReadDependencyStream(dependency, optimizerContext, transformer) {
    function reader() {
        ...
        var availableTransforms = optimizerContext.config.getTransforms();
    ...
    if (cache && dependency.shouldCache() && cacheKey) {
...
function createReadBundleStream(bundle, optimizerContext) {
    ...
    var transformer = transforms.createTransformer(optimizerContext.config.getTransforms());
