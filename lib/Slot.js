        ...
        for (var i=0, len=this.content.length; i<len; i++) {
            ...
            if (content.inline) {
                if (this.contentType === 'js') {
                    output.push('<script ${data.inlineScriptAttrs} marko-body="static-text">' + content.code + '</script>');
                } else if (this.contentType === 'css') {
                    output.push('<style ${data.inlineStyleAttrs} marko-body="static-text">' + content.code + '</style>');
                }
            } else {
