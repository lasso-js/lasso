        ...
        for (var i=0, len=this.content.length; i<len; i++) {
            ...
            if (content.inline) {
                if (this.contentType === 'js') {
                    output.push('<script type="text/javascript" attrs="data.inlineScriptAttrs" c-parse-body-text="false">' + content.code + '</script>');
                } else if (this.contentType === 'css') {
                    output.push('<style type="text/css" attrs="data.inlineStyleAttrs" c-parse-body-text="false">' + content.code + '</style>');
                }
            } else {
