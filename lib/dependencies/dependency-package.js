    ...
    init: function(optimizerContext, callback) {
        ...
        try {
            ...
            if (e.fileNotFound) {
                ...
                return callback(new Error('Optimizer manifest not found for path "' + this.path + '" referenced in "' + (inFile || this.getParentManifestDir()) + '"'));
            } else {
                return callback(new Error('Unable to load optimizer manifest for path "' + this.path + '". Dependency: ' + this.toString() + '. Exception: ' + (e.stack || e)));
            }
        }
