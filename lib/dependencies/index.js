...
function registerType(type, mixins) {
    var isManifestDependency = mixins.isManifestDependency && mixins.isManifestDependency();

    if (!isManifestDependency && (!mixins.read || typeof mixins.read !== 'function')) {
        throw new Error('Unable to register dependency type of "' + type + '". read() is a required function for code dependencies.');
    }
...
exports.normalizeDependency = function(dependency) {
...
registerJavaScriptType('js', require('./Dependency_js'));
registerJavaScriptType('loader-metadata', require('./Dependency_loader-metadata'));
registerStyleSheetType('css', require('./Dependency_css'));
registerType('package', require('./Dependency_package'));
