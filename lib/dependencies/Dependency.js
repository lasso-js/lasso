...
Dependency.prototype = {
    ...
    getPackageManifest: function(context, callback) {
        ...
        if (!manifest) {
            ...
            if (typeof this.loadPackageManifest === 'function') {
                manifest = this.loadPackageManifest(context, function callback(err, dependencies) {
                    if (err) {
                        deferred.reject(err);
                        return;
                    }
    ...
    calculateKey: function(context, callback) {
        ...
        this._key = this.doCalculateKey(context, function(err, lastModified) {
    ...
    doCalculateKey: function(context, callback) {

        if (this.isPackageDependency()) {
            callback(null, this.calculateKeyFromProps());
        } else if (this.isExternalResource()) {
            var url = this.getUrl ? this.getUrl(context) : this.url;
            callback(null, url);
        } else {
            var _this = this;
            ...
            if (context.cache) {
                this.lastModified(context, function(err, lastModified) {
                    if (err) {
                        return callback(err);
                    }
                    ...
                    context.cache.getDependencyChecksum(
                        _this.getPropsKey(),
                        lastModified,
                        doCalculateChecksum,
                        callback);
    ...
    lastModified: function(context, callback) {
        ok(typeof callback === 'function', 'callback is required');
        ...
        this._lastModifiedValue = this.doLastModified(context, function(err, lastModified) {
    ...
    doLastModified: function(context, callback) {
