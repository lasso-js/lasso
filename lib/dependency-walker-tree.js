...
function buildTree(options, callback) {
    ...
    function calculateDependencyKey(dependency) {
    ...
    function loadPackageManifest(dependency, parentNode) {
        queue.push(function(callback) {
            dependency.getPackageManifest(context, function(err, manifest) {
                ...
                handleManifest(manifest, parentNode);
                callback();
            });
        });
    }
    ...
    function handleDependency(dependency, parentNode) {
        if (dependency.isPackageDependency()) {
            ...
            loadPackageManifest(dependency, node);
...
function walk(options, callback) {
    ...
    function done(err) {
        ...
        if (err) {
            ...
            logger.info('Completed walk in ' + (Date.now() - startTime) + 'ms');
            emitter.emit('end');
