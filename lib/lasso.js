...
function doOptimizePage(theLasso, options, lassoContext, callback) {
    ...
    getOptimizerManifestFromOptions(options, theLasso.dependencies, function(err, lassoManifest) {
        ...
        function registerBundle(bundle, async) {
            ...
            if (url) {
                lassoPageResult.addUrl(url, bundle.getSlot(), bundle.getContentType(), async);
            }

            if (!bundle.isExternalResource && bundle.outputFile) {
                lassoPageResult.addFile(bundle.outputFile, bundle.getContentType(), async);
            }
        }
...
Lasso.prototype = {
    ...
    /**
     * A LassoContext is created for each call to Lasso::optimizePage
