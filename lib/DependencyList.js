...
function DependencyList(dependencies, dependencyRegistry, dirname, filename, asyncPackageName) {
    ...
    this._asyncPackageName = asyncPackageName;
}
...
DependencyList.prototype = {
    ...
    _convertDependencies: function() {
        ...
        var convertedDependencies = new Array(this._dependencies.length);
        ...
        for (var i=0, len=this._dependencies.length; i<len; i++) {
            ...
            if (!dependenciesModule.isDependency(d)) {
                ...
                d = this.dependencyRegistry.createDependency(d, this._dirname, this._filename, this._asyncPackageName);
            }

            convertedDependencies[i] = d;
        }

        this._dependencies = convertedDependencies;
    ...
    addDependency: function(config) {
        if (this._converted) {
            config = this.dependencyRegistry.createDependency(config, this._dirname, this._filename, this._asyncPackageName);
        }
