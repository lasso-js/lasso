...
function doLassoResource(theLasso, path, options, lassoContext, callback) {
    ...
    if (isExternalUrl(path)) {
        ...
        var base64Encode = queryString === '?base64' && writer.base64EncodeSupported !== true;
        var utf8Encode = queryString === '?utf8';

        if (base64Encode || utf8Encode) {
            fs.readFile(path, null, function(err, data) {
                if (err) {
                    return done(err);
                }

                var encoding = base64Encode ? 'base64' : 'utf8';

                var dataUrl = 'data:' + mime.lookup(path) + ';' + encoding + ',' + data.toString(encoding);
...
Lasso.prototype = {
    ...
    getJavaScriptDependencyHtml: function(url) {
        return '<script src=' + JSON.stringify(url) + ' ${data.externalScriptAttrs}></script>';
    },

    getCSSDependencyHtml: function(url) {
        return '<link rel="stylesheet" href=' + JSON.stringify(url) + ' ${data.externalStyleAttrs}>';
    },
