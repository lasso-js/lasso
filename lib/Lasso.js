...
function doLassoPage(theLasso, options, lassoContext, callback) {
    ...
    getLassoManifestFromOptions(options, theLasso.dependencies, function(err, lassoManifest) {
        ...
        theLasso.emit('beforeOptimizePage', pluginContext); //legacy
        theLasso.emit('beforeLassoPage', pluginContext); // legacy
...
Lasso.prototype = {
    ...
    getJavaScriptDependencyHtml: function(url) {
        return '<script src="' + escapeXmlAttr(url) + '" attrs="data.externalScriptAttrs"></script>';
    },

    getCSSDependencyHtml: function(url) {
        return '<link rel="stylesheet" href="' + escapeXmlAttr(url) + '" attrs="data.externalStyleAttrs">';
    },
