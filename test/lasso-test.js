...
describe('lasso/index', function() {
    ...
    it('should allow for image URLs in CSS files to be resolved', function(done) {
        ...
        theLasso.lassoPage({
            ...
            .then(function(lassoPageResult) {
                ...
                var actual = writerTracker.getCodeForFilename('testPage-cffaf386.css');
                // console.log(actual);
    ...
    it('should resolve font URLs correctly', function(done) {
        ...
        theLasso.lassoPage({
            ...
            .then(function(lassoPageResult) {
                expect(writerTracker.getCodeForFilename('testPage-ebfc0833.css')).to.equal("@font-face { src: url('Aleo-Regular-6be64eb6.woff'); }");
                lasso.flushAllCaches(done);
            })
            .done();
    });
    ...
    it('should allow for CSP nonce on inline script tags', function(done) {
        ...
        theLasso.lassoPage({
            ...
            .then(function(lassoPageResult) {
                ...
                expect(writerTracker.getOutputFilenames()).to.deep.equal(['testPage.js']);
                expect(body).to.equal('<script src=\"/testPage.js\"></script>\n<script nonce=\"abc\">moduleA_js</script>');
                lasso.flushAllCaches(done);
            })
            .done();
    });
