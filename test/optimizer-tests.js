...
describe('raptor-optimizer' , function() {
    ...
    it('should allow for output filters', function(done) {
        ...
        optimizer.configure({
                ...
                checksumsEnabled: false,
                filters: [
                    require('./src/filters/css-filter1'),
                    require('./src/filters/css-filter2'),
                    require('./src/filters/js-filter1-async'),
                    require('./src/filters/js-filter2-async')
                ]
            }, __dirname, __filename)
            .then(function(pageOptimizer) {
                return pageOptimizer.optimizePage({
    ...
    it('should allow for URLs with the file:// protocol when in-place deployment is enabled', function(done) {
        ...
        //             expect(jsFiles)
        //                 .toEqual([
        //                     path.join(__dirname, 'resources/mixedA/mixedA.js'),
        //                     path.join(__dirname, 'resources/mixedB/mixedB.js'),
        //                     path.join(__dirname, 'resources/optimizer/build/test-optimizer-rhtml-test-template.rhtml-body.js')
        //                 ]);

        //             expect(cssFiles)
        //                 .toEqual([
        //                     path.join(__dirname, 'resources/mixedA/mixedA.css'),
        //                     path.join(__dirname, 'resources/mixedB/mixedB.css')
        //                 ]);
        //         },
        //         done: done    
        //     });
        ...
        optimizer.configure({
            ...
            .then(function(optimizedPage) {
                ...
                expect(writer.getOutputFilenames()).to.deep.equal([
                    'ebay-logo_d481eb85.png',
                    'testPage-b573d3b9.css'
                ]);
            })
            .then(done)
            .fail(done);
